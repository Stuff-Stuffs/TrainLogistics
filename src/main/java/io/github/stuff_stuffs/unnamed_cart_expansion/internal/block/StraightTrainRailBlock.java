...
public class StraightTrainRailBlock extends Block {
    ...
    public void onPlaced(final World world, final BlockPos pos, final BlockState state, @Nullable final LivingEntity placer, final ItemStack itemStack) {
        ...
        final Direction.Axis axis = state.get(FACING);
        final Side side = state.get(SIDE);
        final Direction offset;
        if (side == Side.LEFT) {
            offset = axis == Direction.Axis.X ? Direction.NORTH : Direction.WEST;
        } else {
            offset = axis == Direction.Axis.X ? Direction.SOUTH : Direction.EAST;
        }
        world.setBlockState(pos.offset(offset), state.with(SIDE, side == Side.LEFT ? Side.RIGHT : Side.LEFT));
    }
    ...
    static {
        final VoxelShape leftBase = createCuboidShape(0, 0, 0, 12, 4, 16);
